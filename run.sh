...
do
   echo ${f}
   python ianalysis.py --input ${f}  --group_by subj | tee ${f:0:-12}.log.txt
done

best=($(python model_select.py ${OUTPUTS} ${NUM_TRAIN} ${METRICS}))
...
do
   ...
   do
      ...
      python roberta_fine-tuning.py  --data_dir $DATADIR/${att}/noniids  --model_type  ${topk}  --model_name_or_path ${OUTPUTS}/${topk} --output_dir $dir  --category ${att}  --overwrite_output_dir  --num_train $NUM_TRAIN --per_gpu_train_batch_size $BATCH_SIZE  --eval  --evaldev test
   ...
   # python plot.py $dir $att $METRICS
done